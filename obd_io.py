...
class OBDPort:
     ...
     def __init__(self,portnum,logger,SERTIMEOUT=200,RECONNATTEMPTS=3):
         ...
         logger.info("Opening interface (" + portnum + ")")        
         ...
         logger.info("Interface successfully " + self.port.portstr + " opened")
         ...
         while 1: #until error is returned try to connect
             ...
             response = self.get_result()
             while (response == "atz") or len(response) < 2:
                 response = self.get_result()
             self.ELMver = response
             logger.debug("atz response:" + str(self.ELMver))
             self.send_command("ate0")  # echo off
             logger.debug("ate0 response:" + self.get_result())
             self.send_command("0100")
             ready = self.get_result()
             logger.debug("0100 response1:" + ready)
             if ready=="BUSINIT: ...OK":
                ready=self.get_result()
                logger.debug("0100 response2:" + ready)
                return None
             else:             
                #ready=ready[-5:] #Expecting error message: BUSINIT:.ERROR (parse last 5 chars)
                logger.warn("Connection attempt failed:" + ready)
                time.sleep(5)
                if count==RECONNATTEMPTS:
                  self.close()
                  self.State = 0
                  return None
                logger.info("Connection attempt:" + str(count))
                count=count+1          
     ...
     def send_recieve_command(self, cmd):
         """ Sends a command and removes the command echo from the response"""
         self.send_command(cmd)
         response = self.get_result()
         while (response == cmd) or len(response) < 2:
             response = self.get_result()
         return response
             ...
             while 1:
                 ...
                 if c == '\r' and len(buffer) > 0:
                     ...
                     if buffer != "" or c != ">": #if something is in buffer, add everything
                      buffer = buffer + c
             logger.debug("Got result:" + buffer)
             return buffer
         else:
            logger.debug("NO self.port!" + buffer)
         return None
...
if __name__ == "__main__":
    ...
    logging.basicConfig(level=logging.DEBUG,
                    ...
                    filename='obd_io.log',
                    filemode='w')
    logger = logging.getLogger("vhdl_lib_logger")
    # Now setup console streaming.
    ...
    if (len(sys.argv) > 1):
        port = OBDPort(sys.argv[1], logger, 200, 5)
    else:
        logger.error('Missing serial port command-line argument!')